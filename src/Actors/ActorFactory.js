...
const ActorFactory = function ActorFactory() {
    ...
    this.createActor = function createActor(actorObjectSettings) {
        ...
        /*newActor = */
        this.buildObjects(actorObjectSettings, actor);
    ...
    this.buildObjects = function buildObjects(
        data,
        actor,
        id,
        objMap,
        ...
        if (data.type && data.settings) {
            obj = this.createObjectByType(data.type, data.settings, actor);
        }
        ...
        if (data.children && Array.isArray(data.children)) {
            for (let i = 0; i < data.children.length; i++) {
                let childObj = this.buildObjects(
                    data.children[i],
                    undefined,
                    String(id) + String(i),
                    objMap,
    ...
    this.createObjectByType = function createObjectByType(
        type,
        settings,
        actor
    ) {
    ...
    this.isValidMeshActorSettings = function isValidMeshActorSettings(
        meshData
    ) {
    ...
    this.isValidLightActorSettings = function isValidLightActorSettings(
        lightData
    ) {
        // TODO
    };
